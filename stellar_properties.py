...
class Star:
    ...
    def __init__(self, X=0.55, Y=0.43, Z=0.02):
        """
        ...
        self.setup_sellar_equations()

    def setup_sellar_equations(self):
        """
        ...
        self.properties['luminosity'].set_derivative_relation(
            lambda dd, r, state: 4 * np.pi * r**2 * state["energy"].val[0, -1])
        ...
        self.properties['opticaldepth'].set_derivative_relation(
            lambda dd, r, state: state['opticaldensity'] * state['density'])

        self.properties['temperature'].set_derivative_relation(
            lambda dd, r, state: -min(3 * state['opacity'] * state['density'].val[0, -1] * state['luminosity'].val[0, -1] / (16 * np.pi * state['a'] * C * state['temperature'].val[0, -1]**3 * r**2), (1 - 1 / state['gamma']) * state['temperature'].val[0, -1] * G * state['mass'].val[0, -1] * state['density'].val[0, -1] / (state['pressure'].val[0, -1] * r ^ 2))
        )
        j
        self.properties['density'].set_derivative_relation(
            lambda dd, r, state: -(G * state['mass'].val[0, -1] * state['density'] / r**2 + state['pressure_temp_grad'] * state['temperature'].val[1, -1]) / state['pressure_density_grad']
        )
    ...
    def step_non_de(self):
        ...
        self.properties['pressure'] = np.append(
            ...
            (self.properties['density'].val[0, -1] / Mp)**(5 / 3) / (5 * Me) +
            self.properties['density'] * Kb * self.properties['temperature'] /
            (self.mu * Mp) + a * self.properties['temperature'].val[0, -1] ^
            4 / 3)

        self.properties[
            'pressure_temp_grad'] = self.properties['density'] * Kb / (
                self.mu * Mp
            ) + 4 * a * self.properties['temperature'].val[0, -1]**3

        self.properties['pressure_density_grad'] = (3 * np.pi**2)**(
            2 / 3
        ) * HBAR**2 * (self.properties['density'].val[0, -1] / Mp)**(2 / 3) / (
            3 * Me * Mp) + Kb * self.properties['temperature'] / (self.mu * Mp)
